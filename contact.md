---
layout: default
title: Contact
---

<section style="
  margin-top:0;
  border:1px solid rgba(255,255,255,0.06);
  padding:38px;
  border-radius:14px;
  background:linear-gradient(180deg, rgba(13,17,28,0.60), rgba(6,8,14,0.40));
  backdrop-filter:blur(12px);
  opacity:0;
  animation:contactReveal 1s ease forwards;
">

  <h1 style="
    margin:0 0 14px 0;
    font-size:2rem;
    font-weight:700;
    letter-spacing:-0.02em;
    background:linear-gradient(90deg,#3b82f6,#7c3aed);
    -webkit-background-clip:text;
    -webkit-text-fill-color:transparent;
  ">
    Get in Touch
  </h1>

  <p style="
    color:#b3b7c3;
    max-width:760px;
    font-size:1.08rem;
    line-height:1.65;
    margin:0 0 30px 0;
  ">
    If you'd like to collaborate, share an idea, or talk about something you're building, send a message here.  
    I appreciate thoughtful, well-written messages and reply when it matters.
  </p>


  <!-- GRID -->
  <div class="contact-grid" style="display:grid; grid-template-columns:1fr 360px; gap:24px;">


    <!-- FORM -->
    <div style="
      border:1px solid rgba(255,255,255,0.08);
      padding:22px;
      border-radius:12px;
      background:linear-gradient(180deg, rgba(14,18,28,0.55), rgba(10,12,18,0.25));
      backdrop-filter:blur(10px);
      opacity:0;
      animation:fadeSlide 1s ease forwards;
      animation-delay:0.18s;
    ">

      <!-- YOUR FORMSPREE ID IS HERE AND WORKING -->
      <form id="contactForm" action="https://formspree.io/f/mpwvklre" method="POST">

        <label style="font-size:.95rem; color:#e5e7eb; margin-bottom:6px; font-weight:600;">Your Name</label>
        <input name="name" required style="
          margin-bottom:14px;
          width:100%;
          padding:12px;
          border-radius:8px;
          border:1px solid rgba(255,255,255,0.18);
          background:rgba(0,0,0,0.25);
          color:#e5e7eb;
          font-size:.97rem;
        " />

        <label style="font-size:.95rem; color:#e5e7eb; margin-bottom:6px; font-weight:600;">Email</label>
        <input name="email" type="email" required style="
          margin-bottom:14px;
          width:100%;
          padding:12px;
          border-radius:8px;
          border:1px solid rgba(255,255,255,0.18);
          background:rgba(0,0,0,0.25);
          color:#e5e7eb;
          font-size:.97rem;
        " />

        <label style="font-size:.95rem; color:#e5e7eb; margin-bottom:6px; font-weight:600;">Message</label>
        <textarea name="message" rows="6" required style="
          margin-bottom:14px;
          width:100%;
          padding:12px;
          border-radius:8px;
          border:1px solid rgba(255,255,255,0.18);
          background:rgba(0,0,0,0.25);
          color:#e5e7eb;
          font-size:.97rem;
        "></textarea>

        <div style="display:flex; align-items:center; gap:12px;">
          <button type="submit" style="
            padding:11px 16px;
            border-radius:10px;
            background:linear-gradient(90deg,#3b82f6,#7c3aed);
            border:none;
            color:white;
            font-weight:700;
            cursor:pointer;
            transition:0.2s ease;
          " onmouseover="this.style.transform='translateY(-2px)'"
            onmouseout="this.style.transform='none'">
            Send Message
          </button>

          <div id="formStatus" style="color:#9ca3af; font-size:.95rem;"></div>
        </div>

      </form>

      <!-- AJAX LOGIC -->
      <script>
      (function () {
        const form = document.getElementById('contactForm');
        const status = document.getElementById('formStatus');

        form.addEventListener('submit', async function (e) {
          e.preventDefault();
          status.textContent = 'Sending...';

          const action = form.getAttribute('action');
          const data = new FormData(form);

          try {
            const res = await fetch(action, {
              method: 'POST',
              headers: { 'Accept': 'application/json' },
              body: data
            });

            if (res.ok) {
              form.reset();
              status.style.color = '#7ee787';
              status.textContent = 'Message sent successfully.';
            } else {
              status.style.color = '#ff9b9b';
              status.textContent = 'Message failed to send.';
            }
          } catch (err) {
            status.style.color = '#ff9b9b';
            status.textContent = 'Network error — try again.';
          }

          setTimeout(() => { status.textContent = ''; }, 5000);
        });
      })();
      </script>

    </div>




    <!-- CONTACT INFO -->
    <div style="
      border:1px solid rgba(255,255,255,0.08);
      padding:22px;
      border-radius:12px;
      background:rgba(15,18,28,0.50);
      backdrop-filter:blur(10px);
      opacity:0;
      animation:fadeSlide 1s ease forwards;
      animation-delay:0.32s;
      display:flex;
      flex-direction:column;
      gap:14px;
    ">

      <div style="color:#e5e7eb; font-weight:700; font-size:1.05rem;">Contact Details</div>

      <div style="color:#9ca3af; font-size:.95rem;">Email</div>
      <div style="color:#e5e7eb; font-weight:600;">shanikashameem7@gmail.com</div>

      <div style="height:10px;"></div>

      <div style="color:#9ca3af; font-size:.95rem;">LinkedIn</div>
      <a href="https://www.linkedin.com/in/shanikashameems" target="_blank"
         style="color:#e5e7eb; font-weight:600; text-decoration:none;">
        linkedin.com/in/shanikashameems
      </a>

      <div style="height:10px;"></div>

      <div style="color:#9ca3af; font-size:.95rem;">GitHub</div>
      <a href="https://github.com/shanikashameems" target="_blank"
         style="color:#e5e7eb; font-weight:600; text-decoration:none;">
        github.com/shanikashameems
      </a>

      <div style="flex:1;"></div>

      <div style="font-size:.9rem; color:#9ca3af;">
        I reply to well-structured collaboration requests, technical discussions,  
        and messages that align with the work I'm focused on.
      </div>

    </div>

  </div>




  <!-- FOOTER TEXT -->
  <div style="
    margin-top:26px;
    border-top:1px solid rgba(255,255,255,0.05);
    padding-top:18px;
    color:#9ca3af;
    font-size:.95rem;
    opacity:0;
    animation:fadeSlide 1s ease forwards;
    animation-delay:0.5s;
  ">
    Thanks for taking the time to reach out — if your message needs a reply, I’ll get back to you soon.
  </div>

</section>


<style>
@keyframes contactReveal {
  0% { opacity:0; transform:translateY(20px); filter:blur(12px); }
  70% { opacity:1; transform:translateY(0); filter:blur(0); }
  100% { opacity:1; }
}

@keyframes fadeSlide {
  0% { opacity:0; transform:translateY(14px); filter:blur(10px); }
  70% { opacity:1; transform:translateY(0); filter:blur(0); }
  100% { opacity:1; }
}

/* mobile fallback */
@media (max-width:880px) {
  .contact-grid { grid-template-columns: 1fr !important; }
}
</style>
